const Bank = require('./models/Bank'); // Pastikan path model sesuai
const { MessageEmbed } = require('discord.js');

async function getGlobalBankLeaderboard(message) {
    try {
        // Ambil semua bank dari database
        const banks = await Bank.find();

        if (!banks || banks.length === 0) {
            return message.reply('Tidak ada bank yang ditemukan.');
        }
        ...
        console.error(error);
        message.reply('Terjadi kesalahan saat mengambil leaderboard global bank.');
    }
}
